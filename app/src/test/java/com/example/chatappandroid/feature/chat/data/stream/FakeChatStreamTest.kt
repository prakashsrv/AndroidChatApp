...
class FakeChatStreamTest {
    ...
    @Test
    fun `emit delivers single message to collector`() = runTest {
    ...
    @Test
    fun `emitBurst delivers all messages in order`() = runTest {
    ...
    @Test
    fun `emitBurst of 20 messages delivers all without loss`() = runTest {
    ...
    @Test
    fun `isTyping defaults to false`() = runTest {
    ...
    @Test
    fun `setTyping true updates isTyping`() = runTest {
        stream.isTyping.test {
    ...
    @Test
    fun `setTyping toggles correctly`() = runTest {
        stream.isTyping.test {
            awaitItem() // false
            stream.setTyping(true)
            assertEquals(true, awaitItem())

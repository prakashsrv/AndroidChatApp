...
class OrderingAndDeduplicationTest {
    ...
    @Test
    fun `messages are emitted in clientTimestamp ascending order`() =
        runTest {
            val messages =
                listOf(
    ...
    @Test
    fun `own messages always use clientTimestamp for ordering`() =
        runTest {
            val ownMessage =
                fakeMessage(
                    id = "own",
    ...
    @Test
    fun `rapid burst of messages maintains stable order`() =
        runTest {
            val burst =
                (1..20).map { i ->
    ...
    @Test
    fun `optimistic message id is unique per send`() =
        runTest {
    ...
    @Test
    fun `server echo with same id updates row not creates duplicate`() =
        runTest {
    ...
    @Test
    fun `no duplicate messages in list after rapid inbound burst`() =
        runTest {

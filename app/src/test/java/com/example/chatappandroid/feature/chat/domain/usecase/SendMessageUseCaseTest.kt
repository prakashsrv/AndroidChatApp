...
class SendMessageUseCaseTest {
    ...
    @Test
    fun `invoke inserts pending message before contacting server`() =
        runTest {
    ...
    @Test
    fun `pending message has correct content, senderId and PENDING status`() =
        runTest {
    ...
    @Test
    fun `on server success message is updated to SENT`() =
        runTest {
    ...
    @Test
    fun `on server failure message is updated to FAILED`() =
        runTest {
    ...
    @Test
    fun `pending message is own message`() =
        runTest {
            val slot = slot<Message>()
            coEvery { repository.insertPendingMessage(capture(slot)) } returns Unit
            coEvery { repository.sendMessageToServer(any()) } returns Result.success(fakeConfirmed())
    ...
    @Test
    fun `pending message has a non-zero clientTimestamp`() =
        runTest {
            val slot = slot<Message>()
            coEvery { repository.insertPendingMessage(capture(slot)) } returns Unit
            coEvery { repository.sendMessageToServer(any()) } returns Result.success(fakeConfirmed())
